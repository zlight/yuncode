...
def _footer_link(item) -> rx.Component:
    return rx.el.a(
        item["label"],
        href=item["href"],
        class_name="block text-sm text-slate-400 hover:text-cyan-300 transition-colors py-1.5 font-medium",
    )
...
def _footer_col(title, items) -> rx.Component:
    return rx.el.div(
        rx.el.h4(
            title,
            class_name="text-white text-xs font-bold mb-4 uppercase tracking-wider",
        ),
        rx.el.div(
...
def _social_link(icon: str, label: str) -> rx.Component:
    return rx.el.a(
        ...
        href="#",
        aria_label=label,
        class_name="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:text-cyan-300 hover:border-cyan-500/40 hover:bg-cyan-500/10 transition-all cursor-pointer",
    )
...
def _contact_row(icon: str, text: str) -> rx.Component:
    return rx.el.div(
        ...
        rx.el.span(text, class_name="text-sm text-slate-400 font-medium"),
            ...
            rx.el.div(
                rx.el.div(
                    ...
                    rx.el.p(
                        ...
                        class_name="text-sm text-slate-400 mb-6 max-w-xs leading-relaxed font-medium",
                    ),
                    rx.el.div(
            ...
            rx.el.div(
                rx.el.div(
                    rx.el.div(
                        rx.el.div(
                            rx.el.span(
                                class_name="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse shadow-lg shadow-emerald-400/50"
                            ),
                            rx.el.span(
                                LanguageState.footer_all_operational,
                                class_name="text-xs text-slate-200 font-bold",
                            ),
                            class_name="flex items-center gap-2",
                        ),
                        rx.el.a(
                    ...
                    rx.el.div(
                        ...
                        rx.el.div(
                            rx.el.span(
                                "Visa",
                                class_name="text-[10px] text-slate-300 font-semibold px-2 py-0.5 rounded bg-white/5 border border-white/10",
                            ),
                            rx.el.span(
                                "Mastercard",
                                class_name="text-[10px] text-slate-300 font-semibold px-2 py-0.5 rounded bg-white/5 border border-white/10",
                ...
                rx.el.div(
                    ...
                    rx.el.div(
                        rx.el.a(
                            ...
                            href="#",
                            class_name="text-xs text-slate-500 hover:text-cyan-300 transition-colors font-semibold",
                        ),
                        rx.el.a(
