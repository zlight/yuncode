...
def _console_topbar() -> rx.Component:
    return rx.el.header(
        rx.el.div(
            ...
            rx.el.div(
                rx.el.button(
                    ...
                    class_name=rx.cond(
                        ThemeState.is_dark,
                        "flex items-center px-3 py-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all cursor-pointer",
                        "flex items-center px-3 py-2 rounded-lg bg-neutral-100 border border-neutral-200 hover:bg-neutral-200 transition-all cursor-pointer",
                    ),
                ),
                theme_toggle(),
                rx.el.a(
        ...
        class_name=rx.cond(
            ThemeState.is_dark,
            "fixed top-0 left-0 lg:left-64 right-0 z-30 backdrop-blur-xl bg-slate-950/70 border-b border-white/5",
            "fixed top-0 left-0 lg:left-64 right-0 z-30 backdrop-blur-xl bg-white/85 border-b border-neutral-200",
        ),
    )
...
def console_page() -> rx.Component:
    return rx.el.main(
        ...
        class_name=rx.cond(
            ...
            "font-['Inter'] bg-neutral-50 min-h-screen relative overflow-x-hidden text-neutral-900 antialiased",
        ),
    )
